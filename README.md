# IoT Fleet Telematics & Vehicle Breakdown Predictive Analytics
...
> **🚗 Real-Time Connected Vehicle Sensor Telematics & Component Breakdown Risk Classifier with Streamlit UI**

---

## 📌 Executive Summary & Mathematical Formulation

In IoT / Automotive ML, automated machine learning classification facilitates low-latency decision triage:

$$\mathcal{L}_{\text{logloss}} = -\frac{1}{N} \sum_{i=1}^N \left[ y_i \log(\hat{p}_i) + (1 - y_i) \log(1 - \hat{p}_i) \right]$$

- **Target Classification**: `component_failure_risk`
...
| **Weighted F1 Score** | **0.8404** | > 0.8000 | 🌟 Robust Balance |
| **ROC-AUC Score** | **0.9302** | > 0.8800 | 🌟 High Precision |
| **5-Fold Cross Validation Accuracy** | **0.8559** | Stable K-Fold | Verified |
...
```
fleet-telematics-anomaly-predictor/
├── app.py                     # Streamlit interactive diagnostic dashboard
├── data/
│   ├── raw/
│   │   └── fleet_telematics_anomaly_predictor_dataset.csv   # Raw domain dataset
│   └── processed/
│       └── fleet_telematics_anomaly_predictor_processed.csv # Scaled and preprocessed matrix
├── models/
...
├── notebooks/
│   └── fleet_telematics_anomaly_predictor_pipeline.ipynb    # End-to-end Jupyter Notebook pipeline
├── reports/
...
```bash
git clone https://github.com/ArjunaFransesco/fleet-telematics-anomaly-predictor.git
cd fleet-telematics-anomaly-predictor
python -m venv venv
...
```bash
jupyter notebook notebooks/fleet_telematics_anomaly_predictor_pipeline.ipynb
```
